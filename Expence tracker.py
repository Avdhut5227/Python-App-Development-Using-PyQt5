...
class ExpenseApp(QWidget):
    def __init__(self):
        ...
        self.date_box.setDate(QDate.currentDate())  # Set current date
...
# Move this block outside the class definition
...
# from PyQt5.QtCore import Qt
# from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QLineEdit, QComboBox, QDateEdit,QTableWidget, QVBoxLayout, QHBoxLayout,QTableWidgetItem,QMessageBox
...
#         self.dropdown.addItems(["Food","Transportation","Rent","Shopping","Entainment","Bills","Other"])
...
#     def add_expense(self):
#         date = self.date_box.date().toString("yyyy-MM-dd")  # Fixed date format (it was incorrect, should be MM instead of mm)
#         category = self.dropdown.currentText()
#         amount = self.amount.text()
#         description = self.description.text()
...
#         query.addBindValue(date)
#         query.addBindValue(category)
#         query.addBindValue(float(amount))  # Cast amount to float
#         query.addBindValue(description)

#         if not query.exec_():
#             QMessageBox.critical(self, "Database Error", "Could not add the expense!")
#             return
...
# #Create Database
# database = QSqlDatabase.addDatabase("QSQLITE")
# database.setDatabaseName("expence.db")
# if not database.open():
#     QMessageBox.critical(None,"Error","Could not open the database")
#     sys.exit(1)
...
# query = QSqlQuery()
# query.exec_("""
#             CREATE TABLE IF NOT PRESENT expenses(
#             id INTEGER PRIMARY KEY AUTOINCREMENT,
#             date TEXT,
#             amount REAL,
#             description TEXT
